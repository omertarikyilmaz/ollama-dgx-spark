...
function handleVLMFileSelect(file) {
    ...
    if (isImage) {
        ...
        // Auto-calculate optimal settings when video metadata loads
        previewVideo.onloadedmetadata = function() {
            const duration = previewVideo.duration;
            const settings = calculateOptimalVideoSettings(duration);

            document.getElementById('vlm-frame-interval').value = settings.frameInterval;
            document.getElementById('vlm-max-frames').value = settings.maxFrames;

            // Show info to user
            const durationStr = formatDuration(duration);
            showToast(`Video: ${durationStr} - Optimum ayarlar: ${settings.frameInterval}s aralik, ${settings.maxFrames} kare`, 'info');
        };
    }
